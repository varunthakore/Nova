  ...
  CommitmentKey<E>: CommitmentKeyExtTrait<E>,
{
  ...
  fn get_vector_gen_vk(vk: Self::VerifierKey) -> CommitmentKey<E> {
    vk.ck_v
  }
...
impl<E: Engine> NIFSForInnerProduct<E>
where
  E::GE: DlogGroup,
{
  ...
  CommitmentKey<E>: CommitmentKeyExtTrait<E>,
{
  ...
  pub fn verify(
    ...
    let mut Ls: Vec<<E::GE as DlogGroup>::PreprocessedGroupElement> = self
      .P_L_vec
      .iter()
      .map(|p| {
      ...
      .collect();
    let mut Rs: Vec<<E::GE as DlogGroup>::PreprocessedGroupElement> = self
      .P_R_vec
      .iter()
      .map(|p| {
        Commitment::<E>::decompress(p)

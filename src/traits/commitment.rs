...
pub trait CommitmentEngineTrait<E: Engine>: Clone + Send + Sync {
  ...
  ) -> Vec<<<E as Engine>::GE as DlogGroup>::PreprocessedGroupElement>
  where
    E::GE: DlogGroup;
  ...
  fn get_blinding_gen(
    ck: &Self::CommitmentKey,
  ) -> <<E as Engine>::GE as DlogGroup>::PreprocessedGroupElement
  where
    E::GE: DlogGroup;

  /// Converts a commitment into generators (with no blinding generator)
  fn from_preprocessed(
    com: Vec<<<E as Engine>::GE as DlogGroup>::PreprocessedGroupElement>,
  ) -> Self::CommitmentKey
  where
    E::GE: DlogGroup;
}
